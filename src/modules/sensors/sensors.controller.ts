...
import { SensorInput } from "./sensors.types";
import asyncHandler from "../../utils/asyncHandler";
import { AuthRequest } from "../../middlewares/authMiddleware";

class SensorController {
  // LISTAR TODOS OS SENSORES (SEM PAGINAÇÃO)
  getAllSensors = asyncHandler(
  ...
  // CRIAR SENSOR
  createSensor = asyncHandler(
    async (req: AuthRequest, res: Response) => {
      const user = req.user!;

      const sensorData: SensorInput = req.body;

      const newSensor = await SensorService.create(
        user,
        sensorData,
      );
  ...
  updateSensor = asyncHandler(
    async (req: AuthRequest, res: Response) => {
      ...
      const updatedSensor = await SensorService.update(
        id,
        user,
        sensorData,
      );
      ...
      if (!updatedSensor) {
        return res.status(404).json({
          success: false,
          message:
            "Sensor não encontrado ou não pôde ser atualizado.",
        });
      }
